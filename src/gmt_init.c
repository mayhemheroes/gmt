...
int gmt_parse_R_option (struct GMT_CTRL *GMT, char *arg) {
	...
	GMT->common.R.dim = 2;
	if (i == 6 && (strncmp (GMT->init.module_name, "xyz2grd", 7U) || strncmp (GMT->init.module_name, "grd2xyz", 7U)))
		GMT->common.R.dim = 3;
	else if (i == 6 && strncmp (GMT->init.module_name, "grdinfo", 7U) && !GMT->current.proj.JZ_set && GMT->current.setting.run_mode == GMT_CLASSIC) {
